...
export class EntryController {
    simulate(
        ...
        const distance = (record.point.x * record.point.x) + (record.point.y * record.point.y);
        for (const body of bodies) {
            ...
            if (distance - radius < atmoRadius) {
                ...
                const burnRate = Math.abs((depth-atmoRadius)/-atmoRadius)/10;
                record.mass *= 1-(burnRate*time);

                // Calculate magnitude of current vector
                const magnitude = Math.sqrt(record.vector.x ** 2 + record.vector.y ** 2);
